...
@pytest.fixture
def test_vote(test_post,session,create_test_user):
    new_vote = models.Vote(post_id=test_post[3].id, user_id=create_test_user['id'])
...
def test_vote_twice_twice(authorized_client,test_post,test_vote):
    res = authorized_client.post("/vote/",json={"post_id":test_post[3].id, "dir": 1})

    assert res.status_code == 409
    
    
def test_delete_vote(authorized_client,test_vote,test_post,create_test_user):
    res = authorized_client.post("/vote/",json={"post_id":test_post[3].id, "dir": 0})
    assert res.status_code == 201
    
def test_delete_vote_non_exist(authorized_client,test_post,create_test_user):
    res = authorized_client.post("/vote/",json={"post_id":test_post[3].id, "dir": 0})
